...
from thermo.ambient.models import device, embedding, graph
...
class cPaiNN(device.Module):
    ...
    def __init__(
        ...
        ### Args:
            ...
            - `temp_length (int, optional)`: length of temperature embedding. Defaults to 10.
            - `time_length (int, optional)`: length of time embedding. Defaults to 10.
            - `temperatures (list, optional)`: list of temperatures known to the model. Defaults to [300, 400, 500, 600, 700, 800, 900, 1000].
        """
        ...
        """layers = [
            ...
            embedding.TemperatureEmbedding(
                feature_name="T0",
        ...
        self.net = torch.nn.Sequential(
            ...
            embedding.PositionalEmbedding("t", n_features, max_length=time_length),
            embedding.CombineInvariantFeatures(4*n_features, n_features),
            PaiNNBase(
                n_features=n_features,
    ...
    def forward(self, batch: torch_geometric.data.Batch) -> torch_geometric.data.Batch:
        ...
        """embed_batch = batch.clone()
        embed_batch.x = batch.x0.clone()
        embed_batch = self.embed(embed_batch)

        batch.invariant_node_features = embed_batch.invariant_node_features.clone()
        batch.equivariant_node_features = embed_batch.equivariant_node_features.clone()
        batch.invariant_edge_features = embed_batch.invariant_edge_features.clone()
        batch.edge_index = embed_batch.edge_index.clone()"""

        batch = self.net(batch)

        batch.output = batch.equivariant_node_features.squeeze()
        return batch
